...
namespace SegmentSpace
{
    ...
    int addSegment(const QLineF &crLine);
		...
		int numberBySegment(const Segment *segment) const; // убрать эту функцию
		int segmentCount() const { return iSegmentCount; }
		friend SegmentContainer & segments();

	private:
		//SegmentContainer(); // так компановщк будет выдавать ошибку, хотя у Мейерса именно так. У того же Мейерса в 55 советах говорится, 
